...
1. `valid`: 1 表示有用户请求访问资源，0 表示无蝇虎请求访问资源
2. `user`: 宽度为 2，如果有用户请求访问资源时，输出幸运儿的编号

## 波形

根据上面的需求，可以发现，当同时有多个用户请求访问资源的时候（也就是 `request` 有不止一个位是 `1`时）答案是不唯一的。我们这里选择一种简单的策略：选择编号最小的那一个，这种方式也叫优先级编码器。可以得到如下的波形：
