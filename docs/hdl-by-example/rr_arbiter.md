...
观察波形，我们发现，在上图的 `a` 时刻， `request` 由 `0000` 变成 `0001` 的时候，此时就要输出 `user` 为目前获得访问权的用户编号，但 `user_reg` 保存的是之前的信息，它的更新需要等到 `b` 时刻，因此这里 `user` 还涉及到一些组合逻辑，不能直接 `user <= user_reg`。这里正好是反过来：用 `user_reg` 保存上一周期的 `user`，即 `user_reg <= user`。
...
接下来，让我们用 VHDL/Verilog 来实现上面的逻辑。
